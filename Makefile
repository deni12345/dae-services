...
build:
	...
	@mkdir -p $(BIN_DIR)
	go build -o $(BIN_DIR)/$(APP_NAME) ./cmd/$(MAIN_ENTRY).go

build-linux:
	@echo "Building $(APP_NAME) for Linux..."
	@mkdir -p $(BIN_DIR)
	GOOS=linux GOARCH=amd64 go build -o $(BIN_DIR)/$(APP_NAME)-linux ./cmd/$(MAIN_ENTRY).go

# Development
run:
	@FIRESTORE_EMULATOR_HOST=localhost:8080 GOOGLE_CLOUD_PROJECT=dae-project go run ./cmd/$(MAIN_ENTRY).go
